...
module BYRequest {
    ...
    export function sendShootInfo(targetX:number, targetY:number):boolean {
        let player = game.by.BYData.instance.getPlayerInfo(game.UserService.instance.playerId);
        egret.log("check money : " + player.money + "   " + game.RoomManager.getInstance().curRoomData.enterMinMoney)
    ...
    export function sendShootHitInfo(fishId:string , uuid:string, code:string ) {
        ...
        egret.log("发送击中的鱼 id : " + uuid);
        game.by.BYData.instance.recordPreHitId = uuid;
    }
    ...
    export function changeCanon(canonIndex:number):void {
        ...
        var bytes = obj.toArrayBuffer();
        game.by.BYData.instance.recordCanonId = canonIndex;
