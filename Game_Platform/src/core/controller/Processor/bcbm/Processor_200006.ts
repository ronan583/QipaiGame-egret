...
// }
    export class Processor_200006 extends puremvc.SimpleCommand implements puremvc.ICommand {
        ...
        public execute(notification: puremvc.INotification): void {
            ...
            if(result.BattleFinsh) {
                for(let battleInfo of result.BattleFinsh.battleInfo) {
                    battleInfo.costMoney = battleInfo.costMoney / 1000;
                    battleInfo.totalMoney = battleInfo.totalMoney / 1000;
                }
                for(let battleInfo of result.BattleFinsh.rankInfo) {
                    battleInfo.costMoney = battleInfo.costMoney / 1000;
                    battleInfo.totalMoney = battleInfo.totalMoney / 1000;
                }
            }
