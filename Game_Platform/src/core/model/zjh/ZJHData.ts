module game.zjh {
	...
	export class ZJHBattleFinishData {
		 ...
		 * 牌局中有玩家不在最终强制亮牌名单中
		 */
		public needBrightBtn: boolean = false;

		public battleFinishPlayers: Array<ZJHBattleFinishPlayer>;
		public battleFinishIDs: Array<number>;

		public brightCardInfos: Array<BrightCardInfo> = [];
		public brightInfoIDs: Array<number> = [];
		...
		public setData(data: any): void {
			this.needBrightBtn = false;

			let zjhData = ZJHData.getInstance();
			let selfStatus = zjhData.getSelfZJHPlayer().status;
			...
			this.battleFinishIDs = new Array<number>();
			for (let i = 0; i < data.brightCardInfo.length; i++) {
				...
				if (i == 0) {
					...
					this.brightInfoIDs.push(cardInfo.targetId);
				} else if (i == 1) {
					this.selfCardInfo = cardInfo;
					if(selfStatus != game.zjh.ZJHPlayerStatus.NO_PLAY){
						this.brightCardInfos.push(cardInfo);
						this.brightInfoIDs.push(cardInfo.targetId);
					}	
				} else {
			...
			for (let i = 0; i < data.finishInfo.length; i++) {
				...
				this.battleFinishIDs.push(finalPlayer.playerId);
			}

			for(let i = 0; i < this.battleFinishIDs.length; i++){
				let finalID = this.battleFinishIDs[i];
				if(selfStatus != 0 && this.brightInfoIDs.indexOf(finalID) < 0){
					this.needBrightBtn = true;
					break;
				}
			}

			console.log("###############ZJHBattleFinishData", this);
		}
	}
