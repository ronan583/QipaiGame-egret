	...
	export class QuickStart {
		...
		public quickStart(gameType: number = 0) {
			...
			if (!startItem) {
				...
				//检测更新
				if (game.GameInfoMng.instance.isGameNeedUpdate(this.startItem.gameType)) {
					let typeGroup: game.ChildGameTypeGroup = GameConst.getGroupByGame(this.startItem.gameType);
					this.mediateData = {"gameType": this.startItem.gameType, "gameGroup": typeGroup};
					//对话框
					let text = GameConst.getGameName(this.startItem.gameType) + "有更新，请更新后再进入游戏";
					TipsUI.showTips({
						"text": text,
						"callback": this.mediateUpdateGame,
						"callbackObject": this,
						"tipsType": TipsType.OnlyOk
					});
				}
				// 优先先加载资源
				else if (ModuleLoader.getInstance().IsResLoaded(this.startItem.gameType)) {
					this.onLoadComplete();
				} else {
		...
		private mediateData: any;
		private mediateUpdateGame() {
			game.AppFacade.instance.sendNotification(CommonDataNotify.QUICKSTART_UPDATE_GAME, this.mediateData);
		}

		private moveToUpdateGame() {
			let type = this.startItem.gameType;
			let typeGroup: game.ChildGameTypeGroup = GameConst.getGroupByGame(type);
			console.log("--------游戏类型是 ", typeGroup);
		}
