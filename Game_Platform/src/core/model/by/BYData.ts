module game.by {
	...
	export class FishDeadInfo {
		public fishId:Array<string>;
		public killerId:number;
		public addMoney:number;
		public isOneCatch:boolean = false;
	}
	...
	export class FishInfo {
		...
		public childFishesId:Array<number>;
		public zuHeId:number;
	}
	...
	export class FishTideRound {
		public round:number = 0;
		public fishesId:number = 0;
		public type:number = 0;
		public childFishesId:Array<number> = [];
		public isZuHe:number = 1;
		public fishesCount:number = 10;
		public radius:number = 0;
		public deadFishIds:Array<number> = [];
	}

	export class FishTideInfo {
		public createTime:number = 0;
		public fishTideId:number = 0;
		public isRight:boolean = false;
		public tideRoundArr:Array<FishTideRound> = [];

		public setData(data:any, createTime:number) {
			this.createTime = createTime;
			this.fishTideId = data.fishTideId;
			this.isRight = data.isRight;
			for(let tideInfo of data.fishTideRounds) {
				let round = new FishTideRound();
				round.round = tideInfo.round;
				round.fishesId = tideInfo.fishesId;
				round.type = tideInfo.type;
				round.childFishesId = tideInfo.childFishesId
				round.isZuHe = tideInfo.isZuHe;
				round.fishesCount = tideInfo.fishesCount;
				round.radius = tideInfo.radius;
				round.deadFishIds = tideInfo.deadFishIds;
				this.tideRoundArr.push(round);
			}
		}
	}
	...
	export class BYData {
		...
		public recordCanonId:number = 0;
		public recordPreHitId:string = "";
		...
		public setData(data:any):void {
			...
			for(let player of data.playerInfos) {
				let playerInfo:BYPlayerInfo = this.getPlayerInfo(Number(player.playerId));
				if(!playerInfo) {
					playerInfo = new BYPlayerInfo();
					this.players.push(playerInfo);
					playerInfo.isAdd = true
				} else {
					playerInfo.isAdd = false
				}
				playerInfo.checkFlag = 1;
				
				playerInfo.playerId = Number(player.playerId);
				playerInfo.curBulletMoney = Number(player.curBulletMoney / 1000);
			...
			for(let fish of data.fishesInfos) {
				...
				fishInfo.childFishesId = fish.childFishesId;
				fishInfo.zuHeId = fish.zuHeId;
				this.fishInfos.push(fishInfo);
			}

			egret.log("enter room fish count : " + this.fishInfos.length);
		...
		public setBulletMoney(playerId:number, money:number):void {
			var playerInfo:BYPlayerInfo = this.getPlayerInfo(playerId);
			if(playerInfo) {
				playerInfo.curBulletMoney = Number(money / 1000);
			}
		}
