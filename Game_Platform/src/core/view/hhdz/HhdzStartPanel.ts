module game.hhdz {
	export class HhdzStartPanel extends eui.Component implements eui.UIComponent{
		public constructor() {
			super();
		}
		...
		public initUI(data)
		{
			...
			{
				cornerItem = new HhdzCornerItem();
				cornerItem.scaleX = cornerItem.scaleY = 0.97;
			...
			for(var i = 0 ; i < this.items.length ; i++)
			{
				...
				}else
				{
					if(this.items[i].gameLevel == Number(type))
					{
						...
						{
							this.itemGroup.addChild(this.items[i]);
							HhdzRequest.requestSingleRoom(this.items[i].roomId);
						}
					}else
					{
						if(this.itemGroup.getChildIndex(this.items[i]) != -1)
