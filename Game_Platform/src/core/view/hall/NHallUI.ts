module game {
    export class NHallUI extends ResizePanel {
        ...
        public quickstartUpdate(gameType: number, gameGroup: number){
            switch(gameGroup){
                case 0: 
                    this.onOpenMultiGames();
                    break;
                case 1: 
                    this.onOpenQipaiGames();
                    break;
                case 2: 
                    this.onOpenJiejiGames();
                    break;
            }
            for(let i = 0; i < this.allGameIcons.length; i++){
                let icon = this.allGameIcons[i];
                if(icon.game == gameType){
                    icon.dispatchEvent(new egret.TouchEvent(egret.TouchEvent.TOUCH_TAP));
                }
                break;
            }
        }
