...
class FqzsRecordPanel extends ResizePanel {
	...
	public initData(data: any, money: number) {
		...
		if (this.isBanker) {
			...
			this.totalBetLabel.text = CommonUtil.fixMoneyFormat(Math.abs(data.StakeMoney / 1000));
		} else {
		...
		let type = data.resutlType;
		if(type < 13){
			this.multiIcon.visible = true;
			this.winTypeIcon.visible = true;
			this.tongchiImg.visible = false;
			this.multiIcon.source = "fqzs_zjcx_animal" + type;
			this.winTypeIcon.source = "animald_" + type;
		}else{
			this.multiIcon.visible = false;
			this.winTypeIcon.visible = false;
			this.tongchiImg.visible = true;
			this.tongchiImg.source = "animald_" + type;
		}
	}
}