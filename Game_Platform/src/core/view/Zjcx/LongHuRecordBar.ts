class LongHuRecordBar extends eui.Component {
	...
	public initData(data: any): void {
		...
		if(this.costMoney >= 0){
			...
			if(this.costMoney == 0){
				this.resultIcon.source = "lhdz_zjcx_iconhe";
...
class LongHuRecordPanel extends ResizePanel {
	...
	protected childrenCreated(): void {
		super.childrenCreated();
		this.labelArr = [this.longWinLabel, this.huWinLabel, this.heWinLabel];
		this.nobetIconArr = [this.nobetIcon0, this.nobetIcon1, this.nobetIcon2];
	}
	...
	public init(data: any, money: number) {
		...
		if(data.type1 == 0){
			this.longWinLabel.visible = false;
			this.nobetIconArr[0].visible = true;
		}
		if(data.type2 == 0){
			this.huWinLabel.visible = false;
			this.nobetIconArr[1].visible = true;
		}
		if(data.type3 == 0){
			this.heWinLabel.visible = false;
			this.nobetIconArr[2].visible = true;
		}
	}	
