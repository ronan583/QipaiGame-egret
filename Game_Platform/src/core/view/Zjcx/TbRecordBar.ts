class TbRecordBar extends eui.Component implements eui.UIComponent {
	...
	public initData(data: any): void {
		...
		this.roomLevelImg.source = "tb_zjcx_level" + data.gameLevel;
...
class TbRecordPanel extends ResizePanel {
	...
	public initData(data: any, money: number) {
		...
		if (this.isBanker) {
			...
			this.totalBetLabel.text = CommonUtil.fixMoneyFormat(Math.abs(data.totalStake / 1000));
		} else {
		...
		let type = ""
		if (data.result[0] == data.result[1] && data.result[1] == data.result[2]) {
			type = "豹子";
		} else {
			let pointSum = data.result[0] + data.result[1] + data.result[2];
			if (pointSum <= 10) {
				type = "小";
			} else {
				type = "大";
		...
		this.winType.text = (data.result[0] + data.result[1] + data.result[2]) + "点" + type;


		this.dice0.source = "zjcx_dice_" + data.result[0];
		this.dice1.source = "zjcx_dice_" + data.result[1];
		this.dice2.source = "zjcx_dice_" + data.result[2];
	}
}