class BYCoins extends eui.Component implements  eui.UIComponent {
	...
	public showCoins(playerId:number, bulletMoney:number):void {
		this.bulletValueLabel.text = (bulletMoney).toFixed(3);
	...
	private add():void {
		...
		var bm:number = game.by.BYData.instance.getPlayerInfo(game.UserService.instance.playerId).curBulletMoney * 1000 + 1000 * base;
	...
	private sub():void {
		var base:number = game.RoomManager.getInstance().curRoomData.bottomBet / 10;
		var bm:number = game.by.BYData.instance.getPlayerInfo(game.UserService.instance.playerId).curBulletMoney * 1000 - base * 1000;
		if(bm <= 0) {
