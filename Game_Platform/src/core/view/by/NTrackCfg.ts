module game.by {
    ...
    export class NTrackCfgCache {
        ...
        public static getTrackCfg(trackId:number, stageWidthRatio:number, isFlip:boolean = false):NTrackCfg{
            stageWidthRatio = Math.min(1, stageWidthRatio)
            let trackCfg = null;
            if(NTrackCfgCache.cacheMap.contains("trackid_" + trackId)) {
                trackCfg = NTrackCfgCache.cacheMap.get("trackid_" + trackId);
            } else {
                let trackOBj = RES.getRes("track_" + trackId + "_json");
                trackCfg = new NTrackCfg();
        ...
        public static getTideTrackCfg(leftOrRight:boolean, stageWidthRatio:number, maxRadius):NTrackCfg{
            stageWidthRatio = Math.min(1, stageWidthRatio)
            let trackCfg = new NTrackCfg();
            let item = new NTrackItem(leftOrRight ? -maxRadius : 1624 + maxRadius, 375);
            item.x = item.x * stageWidthRatio;
            item.r = 0;
            item.d = 1624;
            trackCfg.trackItems.push(item);
            item = new NTrackItem(leftOrRight ? 1624 + maxRadius : -maxRadius, 375);
            item.x = item.x * stageWidthRatio;
            item.r = 0;
            item.d = 0;
            trackCfg.trackItems.push(item);
    ...
    export class NTrackCfg {
        ...
        flip() {
            ...
            for(let i = 0;i < this.trackItems.length - 1; i++) {
                ...
                item.y = Global.designRect.y - item.y;
            }
        }
        ...
        public calcAllDistance() {
            this.allDistance = 0;
            for(let i=0;i<this.trackItems.length-1;i++) {
                let item = this.trackItems[i];
                let nitem = this.trackItems[i + 1];
                let d = this.distance(item, nitem)
                item.d = d;
                this.allDistance += d;
            }
        }
