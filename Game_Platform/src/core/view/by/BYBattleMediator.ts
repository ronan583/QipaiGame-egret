    ...
    export class BYBattleMediator extends GameMeditator {
        ...
        public handleNotificationSafe(notification: puremvc.INotification): void {
            ...
            switch (notification.getName()) {
               ...
               case PanelNotify.RESUME_APP:
                    if(this.byBattleScene) {
                        ...
                        if(RoomManager.getInstance().curRoomData) {
                            RoomRequest.sendEnterRoomInfo(game.ChildGameType.BY, RoomManager.getInstance().curRoomData.gameLevel);
                        } else {
                            AppFacade.getInstance().sendNotification(PanelNotify.CLOSE_BY_BATTLE_UI);
                        }
                    }
               break;
               ...
               break;
               case PanelNotify.CREATE_FISH_TIDE:
                    if(this.byBattleScene) {
                        this.byBattleScene.createFishTide(notification.getBody());
                    }
               break;
            }
        }
