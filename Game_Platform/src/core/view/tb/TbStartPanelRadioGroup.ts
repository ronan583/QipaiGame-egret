module game.tb {
	export class TbStartPanelRadioGroup extends eui.Component implements eui.UIComponent{
		public constructor() {
			super();
			this.skinName='resource/eui_skins/tb/TbStartPanelRadioGroupSkin.exml';
		...
		private onSelected(event : egret.TouchEvent)
		{
			...
			if(tempIndex != -1 && tempIndex != this.currIndex)
			{
				...
				AppFacade.getInstance().sendNotification(PanelNotify.UPDATE_TB_RADIO_UI,this.getType(tempIndex));
