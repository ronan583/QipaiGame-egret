    ...
    export class PDKRule {
        ...
        */
        ...
        private static calcPlaneCount(cards: Array<number>): number {
            let map = PDKRule.convertToMap(cards);
            let c = 0;
            for (let k in map) {
                if (map[k] == 3) {
                    c++;
        ...
        public static selectOnTableNoBoom(pCards: Array<number>, tableCard: game.ddz.TableCard, selectCards:Array<number>): DDZExecCards {
            let pokerCards:Array<number> = PDKRule.excludeBoomCards(pCards);
            let cards = PDKRule.analysis(pokerCards, false, false)
            return PDKRule.selectOnTable(cards, tableCard, selectCards)            
        }
        ...
        public static excludeBoomCards(cards:Array<number>):Array<number>{
            ...
            // 移除炸弹的选择项
            let map = PDKRule.convertToMap(pokerCards);
            let needRemove = [];
            for(let k in map) {
                if(map[k] == 4) {
                    // 需要移除k
        ...
        public static selectOnTable(cards: Array<DDZExecCards>, tableCard: game.ddz.TableCard, selectCards:Array<number>) {
            let cardResult: game.pdk.CardCheckResult = game.pdk.PDKPokerUtil.checkType(tableCard.cards);
            ...
            if (tableCard.cardType == game.ddz.CardType.FOUR_TO_TWO) {
                if(selectCards.length >= 6) {
                    ...
                    execCard = PDKRule.findByRoot(CardRoot.FOUR, cardResult.card, cards, 4);
                    if(!execCard) {
                        // 你他妈选了牌 但是你这个4压根打不过对面，看样子 你还是要炸他吧 大哥
                        return PDKRule.findMinBoom(cards);
                    } else {
            ...
            if (tableCard.cardType == game.ddz.CardType.THREE) {
                execCard = PDKRule.findByRoot(CardRoot.THREE, cardResult.card, cards, 3);
            }
            ...
            if (tableCard.cardType == game.ddz.CardType.PLANE) {
                // 飞机到底是几个 需要计算一下子
                let planeCount = PDKRule.calcPlaneCount(tableCard.cards);
                execCard = PDKRule.findByRoot(CardRoot.THREE, cardResult.card, cards, planeCount * 3);
                if (execCard) {
            ...
            if (tableCard.cardType == game.ddz.CardType.MANY_DUAD) {
                //如果是连对的化 那么就找连对-1
                execCard = PDKRule.findLianduiMatch(cardResult.card, cards);
                if (execCard) {
            ...
            if (tableCard.cardType == game.ddz.CardType.SHUN_ZI) {
                execCard = PDKRule.findShunziMatch(cardResult.card + 1, cards, tableCard.cards.length);
            }
        ...
        public static selectOnConditionNoBoom(pokerCards:Array<number>): DDZExecCards {
            let cards = PDKRule.excludeBoomCards(pokerCards);
            let execCardsResults = PDKRule.analysis(cards, false, false);
            return PDKRule.select(execCardsResults);
        }

        public static selectBoom(pokerCards:Array<number>) {
            let execCardsResults = PDKRule.analysis(pokerCards, false, false);
            return PDKRule.findMinBoom(execCardsResults);
        }

        public static selectBoomCardNums(pokerCards:Array<number>):Array<number>{
            let execCardsResults = PDKRule.analysis(pokerCards, false, false);
            let c =  PDKRule.findMinBoom(execCardsResults);
            if(c) {
        ...
        public static select(cards: Array<DDZExecCards>): DDZExecCards {
            ...
            let plane = PDKRule.findPlane(cards);
        ...
        private static findRandomSingleInCards(cards: Array<number>, count: number, suggest: number = 1): Array<number> {
            ...
            let map = PDKRule.convertToMap(clone);
            ...
            for (let arr of collectArr) {
                ...
                do {
                    ...
                    if(target == 15) {
                        ...
                        r = PDKRule.findDirect2InCards(avalibale, target, suggest);
                        if (r.length > 0) {
                            // 找到了
            ...
            for (let arr of collectArr) {
                ...
                do {
                    ...
                    if(target == 15) {
                        ...
                        let findCard = PDKRule.findSingleInCards(avalibale, target, suggest);
                        if (findCard > 0) {
                            // 找到了
                            pairInfo.afterFindSuc();
        ...
        public static searchSingleThreeStage(cards1: Array<number>, cards2: Array<number>, cards3: Array<number>, count:number, suggest: number = 2) {
            ...
            for (let arr of collectArr) {
                avalibale = avalibale.concat(arr);
                if(avalibale.length == 0) continue;
                let r = PDKRule.findRandomSingleInCards(avalibale, count, suggest);
                if(r.length > 0) {
                    count -= r.length;
                    CommonUtil.subArr(avalibale, r);
        ...
        public static searchDuiziThreeStage(cards1: Array<number>, cards2: Array<number>, cards3: Array<number>, count: number) {
            ...
            for (let arr of collectArr) {
                avalibale = avalibale.concat(arr);
                let pairResult = game.PDKRule.searchDuizi(avalibale, count);
        ...
        public static analysis(cards: Array<number>, allow2liandui: boolean = true, allow4shunzi: boolean = true): Array<DDZExecCards> {
            ...
            PDKRule.DebugLogCards(pokerList);
            let copyList = [];
            ...
            let shunziArr = PDKRule.findShunzi(copyList, allow4shunzi);
            ...
            PDKRule.findRoot3(map, pokerList, result);
            PDKRule.findLiandui(map, pokerList, result, allow2liandui);
            for (let r of result) {
                r.logSelf();
            }
            return result;
