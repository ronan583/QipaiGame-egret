class QynnHeadIcon extends eui.Component implements eui.UIComponent {
	...
	public showGoldImm(gold:number) {
	...
	public updateState(state:QYNNPlayerState) {
		if(state == QYNNPlayerState.ob) {
			this.alpha = 0.35;
		} else {
			this.alpha = 1;
		}
		egret.log("============update qynn state : " + this.playerInfo.playerId + " " + state)
	}
