module game.fqzs {
    ...
    export class FqzsBattleScene extends GameScene implements eui.UIComponent {
        ...
        private bankerVipLabel:eui.Label;
        ...
        protected componentInit(): void {
            ...
            this.menuGroup = new MenuGroup(this.menuArrowImg, this.menuContent, "fqzs_battle_json.down_Btn", "fqzs_battle_json.up_Btn")
        ...
        private onSettingBtntap() {
			game.AppFacade.getInstance().sendNotification(PanelNotify.OPEN_SETTING_UI, game.ChildGameType.FQZS);
		}
        ...
        private selectedBetButtonByIndex(index: number) {
            for(var i = 0 ; i < this.betBtnArr.length ; i++)
			{
				...
				if(i == index)
				{
                    ...
                    egret.log("================= record last choose bet : " + index);
        ...
        public updatePlayerInfo() {
            ...
            for (let i = 0; i < roomData.playerInfos.length; i++) {
                ...
                if (playerInfo.postion == 0) {
                    ...
                    this.bankerVipLabel.text = playerInfo.vipLevel.toFixed(0)
                    // }
                    ...
                    this.fqzsData.selfMoney = playerInfo.money;
                }
            }
        ...
        private onStartBet(data, isFirst) {
        ...
        public clearBattleScene() {
            ...
            if(this.waitAmin && this.waitAmin.parent) {
                this.waitAmin.parent.removeChild(this.waitAmin);
            }
        }
        ...
        public onBattleFinish(data) {
            for(let battleInfo of data.battleInfo) {
                if(Number(battleInfo.playerId) == UserService.instance.playerId) {
                    this.fqzsData.selfMoney = battleInfo.totalMoney;
                }
            }
            ...
            for (var i = 0; i < this.betHash.length; i++) {
                ...
                if (i == (this.betHash.length - 1)) {
                    ...
                    this.setTimeOut(()=>{
                        ...
                        this.battleFinish.showResult(data, this.nameLabel.text);
                        // 特殊处理自己的金币
                        this.goldLabel.text = CommonUtil.fixMoneyFormat(this.fqzsData.selfMoney)
                        this.tempBetGold = this.fqzsData.selfMoney;
                        if(FqzsData.isSelfBanker()) {
                            this.bankergoldLabel.text = CommonUtil.fixMoneyFormat(this.fqzsData.selfMoney);
                        }
                    }, 1000);
                }
            }
        ...
        private resultTips(data) {
            ...
            if (data.otherCosMoney != null && data.otherCosMoney != 0) {
                ...
                let fntObj = RES.getRes(data.otherCosMoney < 0 ? "fqzs_lose_fnt" : "fqzs_win_fnt");
                this.playerResult.font = fntObj
            ...
            if (data.battleInfo != null) {
                for (var i = 0; i < data.battleInfo.length; i++) {
                    if (data.battleInfo[i].playerId == UserService.instance.playerId) {
                        this.goldLabel.text = CommonUtil.fixMoneyFormat(data.battleInfo[i].totalMoney);
                        ...
                        if (data.battleInfo[i].money != null && data.battleInfo[i].money != 0) {
                            ...
                            let fntObj = RES.getRes(data.battleInfo[i].money < 0 ? "fqzs_lose_fnt" : "fqzs_win_fnt");
                            this.myResult.font = fntObj;
                    ...
                    if (data.battleInfo[0].money != null && data.battleInfo[0].money != 0) {
                        ...
                        let fntObj = RES.getRes(data.battleInfo[0].money < 0 ? "fqzs_lose_fnt" : "fqzs_win_fnt");
                        this.bankResult.font = fntObj;
        ...
        public handleBankDrawMoney(drawmoney: number, totalmoney: number) {
            ...
            this.goldLabel.text = CommonUtil.fixMoneyFormat(this.tempBetGold);
            this.fqzsData.addMoney(drawmoney);
            this.refreshBtnState();
        }

        private refreshBtnState() {
            if(FqzsData.isSelfBanker() || this.currStatus != FQStatus.startBet) {
