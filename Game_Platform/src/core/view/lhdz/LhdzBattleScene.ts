module game.lhdz {
	...
	export class LhdzBattleScene extends GameScene implements eui.UIComponent {
		...
		private nextTickExecs(data: any) {
			...
			for(let battleInfo of data.stakeInfo) {
				let battlePlayerId = Number(battleInfo.playerId)
				if (battlePlayerId == UserService.instance.playerId) {
					this.tempBetGold = battleInfo.totalMoney;
					this.lhdzData.selfMoney = this.tempBetGold;
				}
				let headIcon = this.getHeadIconByPlayerId(battlePlayerId);
				if (headIcon) {
					headIcon.showImmGold(battleInfo.totalMoney)
				}
			}
		...
		public UpdateHistory(data) {
			this.lhdzHistoryBar.showHistoryNew(data);
		}
		...
		public onStartBet(data, first:boolean = false) {
		...
		private stakeEffect(playerId, type, betNum) {
			...
			let fromPos: egret.Point = from.localToGlobal(from.width / 2, from.width / 2);
			if (playerId == UserService.instance.playerId) {
				// 如果是自己从筹码处飞出去
		...
		public startDealCard(data, flyBet = true, dealCard = true) {
			let fightInfo = null;
			if(data.stakeInfo) {
				fightInfo = data.stakeInfo
			}

			if(data.battleInfo) {
				fightInfo = data.battleInfo
			}
			if(fightInfo) {
				for(let battleInfo of fightInfo) {
					if(Number(battleInfo.playerId) == UserService.instance.playerId) {
						this.lhdzData.selfMoney = battleInfo.totalMoney;
					}
				}
			}
				...
				this.setTimeOut(()=>{
					...
					if (flyBet == true) {
						self.setTimeOut(()=> {
							...
							this.lhdzHistoryBar.prepareForResult();
							this.flyParticle(to, () => {
								this.lhdzHistoryBar.showBoomAnim();
								this.lhdzHistoryBar.showHistoryNew(data);
							})
		...
		public showWinAmin(winTableType:LHDZWinTableType): void {
			...
			if(winTableType == LHDZWinTableType.LONG) {
				playAnim = this.longWinAnim;
			} else if(winTableType == LHDZWinTableType.HU) {
				playAnim = this.huWinAnim;
			}
			if(playAnim) {
				playAnim.visible = true;
				playAnim.playerOnce(()=>{
			...
			if (data != null && data.length > 0) {
				for (let i = 0; i < data.length; i++) {
					...
					if(playerInfo.postion == 0) {
						...
						if(battlePlayerId == UserService.instance.playerId) {
							this.lhdzHistoryBar.updateBankerMoney(this.lhdzData.selfMoney);
							if (tempInfo.money > 0) {
								this.headIcon0.showWin(data[i].money);
							} else if (tempInfo.money < 0) {
								this.headIcon0.showLose(data[i].money);
							}
						} else {
					...
					if (battlePlayerId == UserService.instance.playerId) {
						playerInfo.money = this.lhdzData.selfMoney;
						this.tempBetGold = this.lhdzData.selfMoney;
						this.headIcon0.showImmGold(this.lhdzData.selfMoney);
						LhdzData.selfTotolMoney = this.tempBetGold;
					} 
				}
			}
		...
		public handleBankDrawMoney(drawmoney: number, totalmoney: number) {
			...
			this.headIcon0.showImmGold(this.tempBetGold);
			LhdzData.selfTotolMoney = this.tempBetGold;
			this.lhdzData.addMoney(drawmoney)
			this.refreshBtnState();
		}
