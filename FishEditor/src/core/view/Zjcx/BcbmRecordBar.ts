...
class BcbmRecordPanel extends ResizePanel {
	...
	public initData(data: any, money: number) {
		...
		if (this.isBanker) {
			...
			this.totalBetLabel.text = CommonUtil.fixMoneyFormat(Math.abs(data.allStakeMoney));
		} else {
			this.totalBetIcon.visible = true;
			this.totalBetLabel.text = CommonUtil.fixMoneyFormat(Math.abs(data.myStakeMoney));
		}
