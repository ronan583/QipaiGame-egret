module game {
    export class ZjhBattleUI extends GameScene {
        ...
        public partAdded(partName: string, instance: any): void {
            ...
            if (partName == "testBtn") {
                instance.addEventListener(egret.TouchEvent.TOUCH_TAP, this.showPlayerWinEffect, this);
            }
        }
        ...
        public updateScene() {
            ...
            if (roomData.status != GameStatus.RUNNING) {
                if (roomData.downTime > 0) {
                    ...
                    this.gameStartWait.visible = false;
        ...
        public handleOperationOnEnd(): void {
            ...
            CommonUtil.registerTimeOut(()=>{
                for (let brightCardInfo of finishData.brightCardInfos) {
                    ...
                    console.log("---------------------")
                    console.log("show brightCards on end ", pos, brightCardInfo.targetId);
                    console.log("---------------------")
            ...
            CommonUtil.registerTimeOut(()=>{
                let selfPlayer = ZJHData.getInstance().getPlayerById(UserService.instance.playerId);
                if(selfPlayer.status != 0){  //NO_PLAY
        ...
        public dealCardTween(): void {
            ...
            for (let j = 0; j < 3; j++) {
                ...
                for (let i = 0; i < players.length; i++) {
                    ...
                    let p = cardGroup.globalToLocal(this.dealPos.x, this.dealPos.y);
                    card.x = p.x;
                    card.y = p.y;
        ...
        public showInitCoinAnim(): void {
            ...
            for (let i = 0; i < zjhData.zjhPlayers.length; i++) {
                ...
                /*
                let costMoney = zjhData.zjhPlayers[i].costMoney;
                for(let j=0;j<costMoney / maxBet;j++) {
                    this.showCoinAnim(zjhData.zjhPlayers[i].playerId, maxBet, true);
                }
                */
