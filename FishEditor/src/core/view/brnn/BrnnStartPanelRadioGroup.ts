module game.brnn {
	export class BrnnStartPanelRadioGroup extends eui.Component implements eui.UIComponent{
		public constructor() {
			super();
		}
		...
		private onSelected(event : egret.TouchEvent)
		{
			...
			if(tempIndex != -1 && tempIndex != this.currIndex)
			{
				...
				AppFacade.getInstance().sendNotification(PanelNotify.UPDATE_BRNN_RADIO_UI,this.getType(tempIndex));
