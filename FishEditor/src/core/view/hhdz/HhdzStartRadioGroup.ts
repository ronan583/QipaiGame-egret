module game.hhdz {
	export class HhdzStartRadioGroup extends eui.Component implements eui.UIComponent{
		public constructor() {
			super();
		}
		...
		private onSelected(event : egret.TouchEvent)
		{
			...
			if(tempIndex != -1 && tempIndex != this.currIndex)
			{
				...
				AppFacade.getInstance().sendNotification(PanelNotify.UPDATE_HHDZ_RADIO_UI,this.getType(tempIndex));
