	...
	export class HhdzBattleScene extends GameScene {
		...
		public finishAfterFlyCoins(redWin:boolean, blackWin:boolean, luckyWin:boolean, data:any) {
			...
			for (var i: number = 0; i < this.headInfoGruop.numChildren; i++) {
				var headInfo: any = this.headInfoGruop.getChildAt(i);
				var playerInfo: PlayerInfo = roomData.getPlayerInfoByPos(Number(headInfo.name));
				if (playerInfo == null) {
					continue;
				}
		...
		public onBackhall() {
			...
			if(this.currStatus == HhdzStatus.prepared) {
				// 非下注阶段 自己有没有下注无所谓了
