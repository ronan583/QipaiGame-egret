module game.tgpd {
	export class TgpdBattleScene extends GameScene implements eui.UIComponent {
		...
		public playCurtainFullAnim(){
			//let curLayer = CandyData.instance.curLevel;
			let anim;
		...
		public onTuoguanCancel() {
			...
			this.qxTuoguanBtn.visible = false;
			if (this.isRunning) {
				this.tuoguanBtn.enabled = false;
			} else {
				this.startGameBtn.enabled = true;
				this.increaseLineBtn.enabled = true;
				this.reduceLineBtn.enabled = true;
				this.increaseStakeBtn.enabled = true;
				this.reduceStakeBtn.enabled = true;
			}
		}
		...
		public onTuoguan() {
			...
			this.qxTuoguanBtn.visible = true;
			if (!this.isRunning) {
				// TgpdRequest.requestStartGame(0);
				this.onStartGame();
			}
		...
		public startGameRet(data) {
			...
			this.reduceLineBtn.enabled = false;
			this.increaseLineBtn.enabled = false;
			this.reduceStakeBtn.enabled = false;
			this.increaseStakeBtn.enabled = false;
			//this.tuoguanBtn.enabled = false;
			// this.candyMachine.showCandys();
			//----------new
            this.updateLayerShow();
			// this.updatePassCount();
            // this.refreshPlayerInfo();
		...
		public showAddMoney(data: any): void {
			...
			if (data.candyId % 100 == 6) {
				...
				this.updatePassCount();
				//播放过关水果动画
				// this.currMoneyLabel.visible = false;
				this.updateLayerGroup();
				TgpdSoundPlayer.instance.playerShowPass();
			}	else {
				if (data.money > 0) {
					// candyData.mon += data.money;
					// this.currMoneyLabel.visible = true;
					// this.currMoneyLabel.text = data.money.toFixed(2);
					//this.totalWinMoneyLabel.text = this.currMoney.toFixed(2);
				}
        ...
        // public showRoundWinAnim() {
		// 	this.baofuAnimGroup.visible = true;
		// 	this.baofuAnim.playerOnce(()=>{
		// 		this.baofuAnim.playerTimes(()=>{
		// 			this.setTimeOut(this.hideRoundWinAnim, 1000)
		// 		}, this, 10, "animation_2");
		// 	}, this, "animation_1");
        // }
        // public hideRoundWinAnim() {
		// 	this.baofuAnimGroup.visible = false;
		// 	this.baofuAnim.stop();
        // }			
