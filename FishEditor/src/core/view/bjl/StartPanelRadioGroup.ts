module game.bjl {
	export class StartPanelRadioGroup extends eui.Component implements eui.UIComponent{
		public constructor() {
			super();
		}
		...
		private onSelected(event : egret.TouchEvent)
		{
			...
			if(tempIndex != -1 && tempIndex != this.currIndex)
			{
				...
				AppFacade.getInstance().sendNotification(PanelNotify.UPDATE_BJL_RADIOCHANGE_UI,this.getType(tempIndex));
		...
		public updateStateByIndex(index : number)
		{
			...
			for(var i = 0 ; i < this.radioArr.length ; i++)
			{
				...
				if(index == i)
				{
					...
					// tempImage.source = "bjlGame_json." + nameStr + "_over";
				}else
				{
					ibutton.alpha = 0;
					tempImage.visible = false;
					// tempImage.source = "bjlGame_json." + nameStr + "_out";
				}
			}
		}
