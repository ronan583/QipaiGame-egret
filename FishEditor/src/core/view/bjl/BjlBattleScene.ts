module game.bjl {
	...
	export class BjlBattleScene extends GameScene{
		...
		private validateBetButton(betButton) : boolean {
			...
			{
				if(UserService.instance.money > this.getBetButtonValue(betButton))
				{
					return true;
				}
			}
		...
		public showFinishAfterBetTween(data:any) {
			if(data.battleInfo != null && data.battleInfo != undefined && data.battleInfo.length > 0) {
				...
				for(let i = 0 ; i < data.battleInfo.length ; i++)
				{
					...
					if(this.bjlData.getPlayerById(battlePlayerId) == null)
					{
						continue;
					}
					let headIcon = this.getHeadIconByPos(this.bjlData.getPlayerById(battlePlayerId).position)
					if(headIcon == null)
					{
						continue;
					}
					...
					this.bjlData.getPlayerById(Number(battlePlayerId)).totolMoney = tempInfo.totalMoney;							
					this.bjlData.getPlayerById(battlePlayerId).money = tempInfo.money;
				}
				if(this.bjlData.isSelfBanker()) {
					if(selfData.money > 0)
					{
