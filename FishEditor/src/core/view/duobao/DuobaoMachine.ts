...
class DuobaoMachine extends eui.UILayer{
	...
	private down():void {
		...
		for(let i=0;i<this.candyIconArr.length;i++) {
			...
			needDownSound = true;
			this.candyIconArr[i].downSound = needDownSound;
			CommonUtil.registerTimeOut(()=>{
	...
	private nextDown():void {
		...
		for(let node of nodes) {
			...
			if(icon) {
				...
				if(!completeCallAlready) {
					let money:number = game.duobao.DuobaoData.instance.eliminateInfos[this.curDownIndex].money;
					let count: number = game.duobao.DuobaoData.instance.eliminateInfos[this.curDownIndex].eliminateCount;
		...
		egret.setTimeout(this.delayDown, this, 900);
	}
	...
	private delayDown():void {
		...
		if(duobaoMachineData.nodesList.length > 0 && this.curDownIndex <duobaoMachineData.nodesList.length) {
			egret.setTimeout(this.nextDown, this, 600);
		} else {
			egret.setTimeout(()=>{if(this.endFunc) {
				this.endFunc.call(this.endFuncObj);
			}},this,1200);
			//------showRoundWin
			egret.setTimeout(()=>{
